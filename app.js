...
app.post('/admin', function(req, res){
    ...
    //  res.render('admin.html');
    //} else {
    //else if (req.originalUrl.match("ˆ/admin?username")) {
      console.log(req.body);
      ...
      // create the user
      create_user(username, password, password, email, phone, function(err, username) {
        if (err) {
          //res.render('signup.html', {error: err});
          res.send(JSON.stringify({status: err}));
        } else {
          var re = {status:"User added successfully!"};
          res.send(JSON.stringify(re));
        }
      });
    //}
